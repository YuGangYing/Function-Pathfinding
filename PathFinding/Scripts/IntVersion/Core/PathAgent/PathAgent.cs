...
namespace BlueNoah.PathFinding.Int
{
    ...
    public class PathAgent
    {
        ...
        public List<Node> StartFind(Vector3Int targetPos)
        {
                ...
                if (node != targetNode)
                {
                    ...
                    for (int i = 0; i < node.neighbors.Count; i++)
                    {
                        if (node.neighbors[i].isOpen != agentIndex && node.neighbors[i].isClose != agentIndex && !node.neighbors[i].IsBlock)
                        {
                            ...
                            //Calculate H
                            node.neighbors[i].H = Mathf.Abs(targetNode.x - node.neighbors[i].x) + Mathf.Abs(targetNode.z - node.neighbors[i].z);
                            //Calculate F
